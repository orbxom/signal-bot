...
async function handleGetObservations(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (!geohash || typeof geohash !== 'string' || geohash.length < 6) {
    return {
      content: [{ type: 'text', text: 'Missing or invalid geohash (need at least 6 characters).' }],
      isError: true,
    };
  }
  ...
  if (d.temp != null)
    lines.push(`Temperature: ${d.temp}°C${d.temp_feels_like != null ? ` (feels like ${d.temp_feels_like}°C)` : ''}`);
...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<ToolResult> {
  try {
    ...
    return { content: [{ type: 'text', text: `Weather API error: ${getErrorMessage(error)}` }], isError: true };
  }
}

runMcpServer({
  name: 'signal-bot-weather',
  tools: TOOLS,
  handleToolCall,
  onInit() {
